...
11. Overseer отправляет агенту ЦЕЛЫЙ СПРИНТ за раз. Агент выполняет все задачи из current_sprint.md последовательно, без промежуточных отчётов. По завершении выводит RALPH_SPRINT_DONE. Затем overseer расспрашивает о каждой задаче отдельно для сбора RALPH_RESULT.
12. После закрытия спринта (аудит OK + сбор отчётов + git commit) overseer автоматически перезапускает Claude Code для сброса контекста.
13. Каждый спринт привязан к session ID (sprint_sessions.json). При крэше overseer делает resume той же сессии, чтобы Claude помнил контекст.
...
- `sprint_sessions.json` — привязка спринтов к session ID для resume при крэше
...
## Протокол RALPH_SPRINT_DONE
После выполнения ВСЕХ задач спринта агент выводит маркер:
```
RALPH_SPRINT_DONE
```
Overseer затем поочерёдно расспрашивает агента о каждой задаче для сбора RALPH_RESULT.
